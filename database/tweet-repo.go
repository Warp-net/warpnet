...
func (repo *TweetRepo) Quoting(tweetId string, limit *uint64, cursor *string) ([]domain.Tweet, string, error) {
	...
	out := make([]domain.Tweet, 0, len(items))
...
func (repo *TweetRepo) AppendQuoting(quotedId string, quoteTweet domain.Tweet) error {
	...
	if quoteTweet.Id == "" {
		return local.DBError("empty quote tweet id")
	}
	key := local.NewPrefixBuilder(TweetsNamespace).
		AddSubPrefix("QUOTING").
		AddRootID(quotedId).
		AddReversedTimestamp(quoteTweet.CreatedAt).
		AddParentId(quoteTweet.Id).
		Build()

	bt, err := json.Marshal(quoteTweet)
	if err != nil {
		return err
	}
