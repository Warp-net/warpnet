...
func (s *NotificationsRepoTestSuite) TestListSince_ReturnsOnlyNewer() {
	...
	for i, id := range ids {
		s.Require().NoError(s.repo.Add(domain.Notification{
			...
			Text:      "n",
			UserId:    userId,
			CreatedAt: time.Now().Add(-time.Duration(len(ids)-i) * time.Second),
		}))
	}
...
func (s *NotificationsRepoTestSuite) TestListSince_EmptyWatermarkFallsBackToList() {
	userId := uuid.New().String()
	for i := 0; i < 2; i++ {
		s.Require().NoError(s.repo.Add(domain.Notification{
			Id:        uuid.New().String(),
			...
			Text:      "n",
			UserId:    userId,
			CreatedAt: time.Now().Add(-time.Duration(2-i) * time.Second),
		}))
	}

	nots, cursor, err := s.repo.ListSince(userId, "", nil)
	s.Require().NoError(err)
	s.Len(nots, 2)
	s.Equal("end", cursor)
}

func (s *NotificationsRepoTestSuite) TestListSince_RespectsLimit() {
	userId := uuid.New().String()
	for i := 0; i < 5; i++ {
		s.Require().NoError(s.repo.Add(domain.Notification{
			Id:        uuid.New().String(),
	...
	limit := uint64(2)
	nots, _, err := s.repo.ListSince(userId, "never-seen", &limit)
	s.Require().NoError(err)
	s.Len(nots, 2)
}

func (s *NotificationsRepoTestSuite) TestListSince_MissingUserId() {
	_, _, err := s.repo.ListSince("", "x", nil)
	s.Error(err)
	s.Contains(err.Error(), "missing user id")
}
