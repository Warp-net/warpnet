...
func (repo *ChatRepo) IsParticipants(ownerId, otherUserId string) bool {
	if ownerId == "" || otherUserId == "" {
		return false
	}
	...
	// The composed ID carries only the random tail of each ULID, so a hit on
	// the key is not proof of membership on its own.
	return isChatMember(chat, ownerId) && isChatMember(chat, otherUserId)
}

func isChatMember(chat domain.Chat, userId string) bool {
	return chat.OwnerId == userId || chat.OtherUserId == userId
}
