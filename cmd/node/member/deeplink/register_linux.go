...
// Writes ~/.local/share/applications/warpnet.node, refreshes the node
// MIME cache (update-node-database) and asks xdg-mime to make it default.
func registerPlatform() error {
	...
	desktopPath := filepath.Join(appsDir, "warpnet.node")
...
`, exe, Scheme)
	...
	if err := os.WriteFile(desktopPath, []byte(contents), 0o644); err != nil { //nolint:mnd,gosec
		return fmt.Errorf("deeplink: write .node: %w", err)
	}
	log.Infof("deeplink: wrote %s (Exec=%q)", desktopPath, exe)

	// Cache refresh stays best-effort: some minimal images ship
	// without update-node-database, but the binding below can
	// still take.
	if err := runShort("update-node-database", appsDir); err != nil {
		log.Warnf("deeplink: update-node-database: %v", err)
	}

	if err := runShort("xdg-mime", "default", "warpnet.node", "x-scheme-handler/"+Scheme); err != nil {
	...
	if got != "warpnet.node" {
		return fmt.Errorf("deeplink: x-scheme-handler/%s resolves to %q, expected warpnet.node", Scheme, got) //nolint:err113 // diagnostic, not matched by callers
	}
