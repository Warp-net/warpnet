...
func NewMemberNode(
	...
	pubsubService := memberPubSub.NewPubSub(ctx, limits, pubSubHandlers...)
...
func (m *MemberNode) Start() (err error) {
	...
	m.mw = middleware.NewWarpMiddleware(m.node.Node().ID(), m.aliasesRepo, m.limits)
	...
	m.rating.Enforce(m.limits, m.node)
