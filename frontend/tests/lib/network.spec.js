...
describe('isBridgedUser', () => {
  it('classifies a ULID id without a network tag as warpnet', () => {
    expect(isBridgedUser({ id: ULID })).toBe(false);
  });

  it('classifies a lowercase ULID id as warpnet', () => {
    expect(isBridgedUser({ id: ULID.toLowerCase() })).toBe(false);
  });

  it('classifies a fediverse handle without a network tag as mastodon', () => {
    expect(isBridgedUser({ id: 'bob@mastodon.social' })).toBe(true);
  });

  it('prefers the network tag over the id format', () => {
    expect(isBridgedUser({ id: ULID, network: 'mastodon' })).toBe(true);
    expect(isBridgedUser({ id: 'bob@mastodon.social', network: 'warpnet' })).toBe(false);
  });

  it('treats testnet and mainnet tags as warpnet', () => {
    expect(isBridgedUser({ id: 'anything', network: 'testnet' })).toBe(false);
    expect(isBridgedUser({ id: 'anything', network: 'mainnet' })).toBe(false);
  });

  it('handles a missing user', () => {
    expect(isBridgedUser(null)).toBe(false);
  });
});

describe('isBridgedTweet', () => {
  it('classifies by the network tag first', () => {
    expect(isBridgedTweet({ user_id: ULID, network: 'mastodon' })).toBe(true);
    expect(isBridgedTweet({ user_id: 'bob@mastodon.social', network: 'warpnet' })).toBe(false);
    expect(isBridgedTweet({ user_id: 'x', network: 'testnet' })).toBe(false);
    expect(isBridgedTweet({ user_id: 'x', network: 'mainnet' })).toBe(false);
  });

  it('falls back to the user_id shape without a tag', () => {
    expect(isBridgedTweet({ user_id: 'bob@mastodon.social' })).toBe(true);
    expect(isBridgedTweet({ user_id: ULID })).toBe(false);
  });

  it('handles a missing tweet', () => {
    expect(isBridgedTweet(null)).toBe(false);
  });
});

describe('bridgedInstance', () => {
  it('extracts the instance from a fediverse handle', () => {
    expect(bridgedInstance('bob@mastodon.social')).toBe('mastodon.social');
  });

  it('returns empty for non-handles', () => {
    expect(bridgedInstance(ULID)).toBe('');
