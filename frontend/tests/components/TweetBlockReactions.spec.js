...
describe('TweetBlock reactions', () => {
  ...
  it('reacts with the chip’s emoji when another chip is clicked', async () => {
    warpnetService.reactToTweet.mockResolvedValue({count: 3, reactions: {'🔥': 1, '❤️': 2}});
    renderTweet();
    ...
    expect(warpnetService.unreactTweet).not.toHaveBeenCalled();
  ...
  it('takes the reaction back when the viewer clicks the one they hold', async () => {
    warpnetService.unreactTweet.mockResolvedValue({count: 2, reactions: {'🔥': 1, '❤️': 1}});
    renderTweet();

    await waitFor(() => expect(chipFor('🔥')).toBeTruthy());
    await fireEvent.click(chipFor('🔥'));

    await waitFor(() => expect(warpnetService.unreactTweet).toHaveBeenCalledWith('t1', 'author1'));
    expect(warpnetService.reactToTweet).not.toHaveBeenCalled();
  });
  ...
  it('lets the node’s my_reaction outrank a stale local cache', async () => {
    ...
    warpnetService.getReactorEmoji.mockResolvedValue('❤️');
    renderTweet();
