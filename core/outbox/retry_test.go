...
func (s *fakeSender) RedeliverStream(nodeId string, path stream.WarpRoute, payload []byte, msgID string) ([]byte, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.gotIDs = append(s.gotIDs, msgID)
...
func TestFlushStopsOnOfflinePreservingFIFO(t *testing.T) {
	...
	require.Len(t, sender.gotIDs, 2, "stopped after offline, third not attempted")
}
