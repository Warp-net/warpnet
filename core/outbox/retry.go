...
// Sender redelivers a queued request. Implementations must NOT re-enqueue on
// failure (that would loop); the retry service owns the queue lifecycle.
...
func (s *RetryService) flush(nodeId string) {
	...
	for _, entry := range entries {
		...
		_, err := sender.RedeliverStream(nodeId, stream.WarpRoute(entry.Route), entry.Payload, entry.Id)
		switch {
		case err == nil:
