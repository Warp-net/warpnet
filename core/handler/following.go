...
func StreamFollowHandler(
	...
	return func(buf []byte, s warpnet.WarpStream) (any, error) {
		...
		if isMeFollowed { //nolint:nestif
			...
			if err := warpnet.VerifyAuthorship(s, ev.FollowerId, followerUser.NodeId); err != nil {
				return nil, err
			}
		...
		if err := warpnet.VerifyAuthorship(s, ev.FollowerId, owner.NodeId); err != nil {
			return nil, err
		}
...
func StreamUnfollowHandler(
	...
	return func(buf []byte, s warpnet.WarpStream) (any, error) {
		...
		if isMeUnfollowed {
			...
			if err := warpnet.VerifyAuthorship(s, ev.FollowerId, follower.NodeId); err != nil {
				return nil, err
			}
...
func StreamGetFollowersHandler(
	...
	return func(buf []byte, s warpnet.WarpStream) (any, error) {
		...
		owner := authRepo.GetOwner()
		isOwnFollowersRequest := ev.UserId == owner.UserId
		if isOwnFollowersRequest {
...
func StreamGetFollowingsHandler(
	...
	return func(buf []byte, s warpnet.WarpStream) (any, error) {
		...
		owner := authRepo.GetOwner()
		isOwnFollowingsRequest := ev.UserId == owner.UserId
		if isOwnFollowingsRequest { // get own followings
