...
func TestAuthMiddleware_PrivateRouteDeniedForForeignPeer(t *testing.T) {
	...
	mw := NewWarpMiddleware(ownNodeId, nil, nil)
	defer mw.Close()
...
func TestAuthMiddleware_PrivateRouteAllowedForPairedDevice(t *testing.T) {
	...
	unpaired := NewWarpMiddleware(ownNodeId, stubAliases{}, nil)
	...
	paired := NewWarpMiddleware(ownNodeId, stubAliases{ids: []string{device.String()}}, nil)
	defer paired.Close()
...
func TestAuthMiddleware_UnknownDeviceStaysLockedOut(t *testing.T) {
	...
	mw := NewWarpMiddleware(ownNodeId, stubAliases{ids: []string{paired.String()}}, nil)
	defer mw.Close()
...
func TestAuthMiddleware_DeviceLookupFailureDenies(t *testing.T) {
	...
	mw := NewWarpMiddleware(ownNodeId, stubAliases{err: errors.New("db is closed")}, nil)
	defer mw.Close()
