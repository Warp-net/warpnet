...
func TestAuthMiddleware_PrivateRouteAllowedForPairedDevice(t *testing.T) {
	...
	unpaired := NewWarpMiddleware(ownNodeId, stubAliases{}, ratelimit.NewStreamLimiter(ratelimit.Settings{}, nil))
	...
	paired := NewWarpMiddleware(ownNodeId, stubAliases{ids: []string{device.String()}}, ratelimit.NewStreamLimiter(ratelimit.Settings{}, nil))
	defer paired.Close()
...
func TestAuthMiddleware_UnknownDeviceStaysLockedOut(t *testing.T) {
	...
	mw := NewWarpMiddleware(ownNodeId, stubAliases{ids: []string{paired.String()}}, ratelimit.NewStreamLimiter(ratelimit.Settings{}, nil))
	defer mw.Close()
...
func TestAuthMiddleware_DeviceLookupFailureDenies(t *testing.T) {
	...
	mw := NewWarpMiddleware(ownNodeId, stubAliases{err: errors.New("db is closed")}, ratelimit.NewStreamLimiter(ratelimit.Settings{}, nil))
	defer mw.Close()
