...
func TestAuthMiddleware_PrivateRouteAllowedForPairedDevice(t *testing.T) {
	...
	unpaired := NewWarpMiddleware(ownNodeId, stubAliases{}, warpnet.NewPeerStandings())
	...
	paired := NewWarpMiddleware(ownNodeId, stubAliases{ids: []string{device.String()}}, warpnet.NewPeerStandings())
	defer paired.Close()
...
func TestAuthMiddleware_UnknownDeviceStaysLockedOut(t *testing.T) {
	...
	mw := NewWarpMiddleware(ownNodeId, stubAliases{ids: []string{paired.String()}}, warpnet.NewPeerStandings())
	defer mw.Close()
...
func TestAuthMiddleware_DeviceLookupFailureDenies(t *testing.T) {
	...
	mw := NewWarpMiddleware(ownNodeId, stubAliases{err: errors.New("db is closed")}, warpnet.NewPeerStandings())
	defer mw.Close()
