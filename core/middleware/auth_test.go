...
func TestAuthMiddleware_PrivateRouteAllowedForPairedDevice(t *testing.T) {
	...
	unpaired := NewWarpMiddleware(ownNodeId, stubAliases{}, allowing{}, ratelimit.Settings{})
	...
	paired := NewWarpMiddleware(ownNodeId, stubAliases{ids: []string{device.String()}}, allowing{}, ratelimit.Settings{})
	defer paired.Close()
...
func TestAuthMiddleware_UnknownDeviceStaysLockedOut(t *testing.T) {
	...
	mw := NewWarpMiddleware(ownNodeId, stubAliases{ids: []string{paired.String()}}, allowing{}, ratelimit.Settings{})
	defer mw.Close()
...
func TestAuthMiddleware_DeviceLookupFailureDenies(t *testing.T) {
	...
	mw := NewWarpMiddleware(ownNodeId, stubAliases{err: errors.New("db is closed")}, allowing{}, ratelimit.Settings{})
	defer mw.Close()
