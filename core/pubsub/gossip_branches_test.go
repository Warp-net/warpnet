...
func TestPublishRaw(t *testing.T) {
	...
	t.Run("refuses when the gossip is down", func(t *testing.T) {
		down := NewGossip(context.Background(), warpnet.NewPeerLimiter())
...
func TestRunPreSubscribesHandlers(t *testing.T) {
	...
	node := newLiveNode(t)
	g := NewGossip(ctx, warpnet.NewPeerLimiter(), TopicHandler{
...
func TestRunRejectsAnInvalidPreSubscription(t *testing.T) {
	...
	node := newLiveNode(t)
	g := NewGossip(ctx, warpnet.NewPeerLimiter(), TopicHandler{TopicName: "", Handler: func([]byte) error { return nil }})
