...
func TestNewGossip(t *testing.T) {
	g := NewGossip(context.Background(), warpnet.NewPeerStandings(), TopicHandler{
		TopicName: "topic-a",
...
func runningGossip(t *testing.T, handlers ...TopicHandler) (*Gossip, *liveNode) {
	...
	node := newLiveNode(t)
	g := NewGossip(ctx, warpnet.NewPeerStandings(), handlers...)
...
func TestGossip_RunPeerInfoPublishingStopsWithContext(t *testing.T) {
	...
	g := NewGossip(ctx, warpnet.NewPeerStandings())
	require.NoError(t, g.Run(node))
