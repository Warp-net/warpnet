...
package site.warpnet.warpdroid.components.account.list
...
class ListsForAccountViewModel @Inject constructor(
    ...
    fun load(accountId: String?) {
        ...
        viewModelScope.launch {
            runCatching {
                val all = warpnetApi.getLists().getOrThrow()
                var includes: List<MastoList> = emptyList()
                if (accountId != null) {
                    includes = warpnetApi.getListsIncludesAccount(accountId).getOrThrow()
                }
    ...
    fun addAccountToList(accountId: String, listId: String) {
        ...
        viewModelScope.launch {
            warpnetApi.addAccountToList(listId, listOf(accountId))
                .onSuccess {
                    _states.emit(
    ...
    fun removeAccountFromList(accountId: String, listId: String) {
        _actionError.resetReplayCache()
        viewModelScope.launch {
            warpnetApi.deleteAccountFromList(listId, listOf(accountId))
                .onSuccess {
                    _states.emit(
                        _states.first().map { state ->
                            if (state.list.id == listId) {
