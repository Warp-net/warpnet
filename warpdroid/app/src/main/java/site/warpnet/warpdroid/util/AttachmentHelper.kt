...
fun List<Attachment>.aspectRatios(): List<Double> {
    return map { attachment ->
        ...
        if (size.aspect > 0) return@map size.aspect.toDouble().coerceIn(0.5, 2.0)
        if (size.width == 0 || size.height == 0) return@map 1.7778
        (size.width.toDouble() / size.height).coerceIn(0.5, 2.0)
    }
}

fun List<Attachment>.hasPreviewableAttachment(): Boolean {
    return any { attachment ->
        attachment.type != Attachment.Type.AUDIO && attachment.type != Attachment.Type.UNKNOWN
    }
}
