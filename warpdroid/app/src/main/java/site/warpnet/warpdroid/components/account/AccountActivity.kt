...
import site.warpnet.warpdroid.view.showMuteAccountDialog
...
class AccountActivity : BottomSheetActivity(), ActionButtonActivity, MenuProvider, LinkListener {
    ...
    private fun updateBadges() {
        ...
        if (loadedAccount?.bot == true) {
            val badgeView =
                getBadge(
                    getColor(R.color.warpdroid_grey_50),
        ...
        loadedAccount?.roles?.forEach { role ->
            val badgeColor = if (role.color.isNotBlank()) {
                ...
                getColor(R.color.warpdroid_blue)
            }
    ...
    private fun onRelationshipChanged(relation: Relationship) {
        ...
        // it's also now supported in Warpnet 3.3.0rc but called notifying and use different API call
        if (!viewModel.isSelf &&
